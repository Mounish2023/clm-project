...
# @app.post("/api/v1/amendments/{workflow_id}/party-response")
# async def submit_party_response(
#     workflow_id: str,
#     response: PartyResponseUpdate,
#     background_tasks: BackgroundTasks,
#     db: Session = Depends(get_db)
# ):
#     """
#     Submit a party's response to an amendment proposal
#     """
#     try:
#         print(f"📝 API: Party {response.party_id} submitting response for workflow {workflow_id}")
        
#         # This would typically update the workflow state and trigger continuation
#         # For now, we'll store the response and notify via WebSocket
        
#         # Update database
#         amendment = db.query(Amendment).filter(Amendment.id == workflow_id).first()
#         if amendment:
#             if not amendment.approvals:
#                 amendment.approvals = {}
#             amendment.approvals[response.party_id] = response.model_dump()
#             db.commit()
...
#         background_tasks.add_task(resume_workflow_if_needed, workflow_id)
        
#         return {
#             "message": f"Response from {response.party_id} recorded successfully",
#             "workflow_id": workflow_id,
#             "party_id": response.party_id,
#             "status": response.status
#         }
...
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=f"Failed to submit party response: {str(e)}")
...
# async def resume_workflow_if_needed(workflow_id: str):
#     """
#     Background task to resume workflow if all parties have responded
#     """
#     try:
#         status = await get_amendment_status(workflow_id)
        
#         # Check if we should resume workflow
#         if status.get("status") == "under_review":
#             # This would check if all parties have responded
#             await orchestrator.resume_workflow(workflow_id)
            
#     except Exception as e:
#         print(f"Resume workflow error: {str(e)}")
...
#     print("🚀 Contract Amendment Orchestrator API starting up...")
...
#     """Cleanup on shutdown"""
#     print("🔄 Contract Amendment Orchestrator API shutting down...")
